...
void pointer_reading_work() {
	while (true) {
		try {
			auto LevelName = mem::readString(baseaddress + off::lastVisibleTime , 64);
			Pointers->local_player = c_player(mem::Read<DWORD_PTR>(baseaddress + off::localPlayer));
...
void weapon_glow()
{
	if (settings::weapon_glow && Pointers->local_player.valid()) {
		uint64_t actWeaponID = mem::Read<uint64_t>(Pointers->local_player.get_address() + off::viewModel) & 0xFFFF;
		if (!actWeaponID) return;
		uint64_t currentWeapon = mem::Read<uint64_t>(baseaddress + off::entityList + (actWeaponID * 0x20));
		...
		if (0 == 5 || 0 == 6)
		{
			auto highlightsettings = mem::Read<uint64_t>(baseaddress + off::glow_highlights);
...
void stop_glow(uintptr_t entity)
{
	...
	uint64_t settings = mem::Read<uint64_t>(entity + off::glow_highlights);
...
void player_reading_work() {
	while (true) {
		...
		for (int i = 0; i <= Pointers->size; i++) {
			const auto entity_addr = mem::Read<DWORD_PTR>(baseaddress + off::entityList + (i * 0x20));
			...
			if (signifier.find("prop_survival") != std::string::npos || signifier.find("prop_death_box") != std::string::npos) {
				...
				const int index = mem::Read<int>(entity_addr + 0x15d4); // m_customScriptInt
...
void loop()
{
	...
	for (size_t i = 0; i < EntityList.size(); ++i) {
		...
		if (settings::health_shield && player_entity.distance <= 100)
		{
			...
			float shieldHeight = barHeight * 0.35f;
			float padding = 2.0f;

			float x = head1.x;
			float y = y_offset;

			int health = player_entity.player.i_health();
			int shield = player_entity.player.i_shield();
			int maxShield = player_entity.player.i_max_shield();
			int ColorMode = 0; // 0 = MaxShield, 1 = CurrentShield

			// Determine shield color
			ImColor shieldBarColor;
			if (ColorMode == 0) // MaxShield
			{
				if (maxShield == 50) shieldBarColor = ImColor(168, 168, 168, 255);
				else if (maxShield == 75) shieldBarColor = ImColor(39, 178, 255, 255);
				else if (maxShield == 100) shieldBarColor = ImColor(206, 59, 255, 255);
				else if (maxShield == 125) shieldBarColor = ImColor(219, 2, 2, 255);
			}
			else if (ColorMode == 1) // CurrentShield
			{
				if (shield <= 50) shieldBarColor = ImColor(168, 168, 168, 255);
				else if (shield <= 75) shieldBarColor = ImColor(39, 178, 255, 255);
				else if (shield <= 100) shieldBarColor = ImColor(206, 59, 255, 255);
				else if (shield <= 125) shieldBarColor = ImColor(219, 2, 2, 255);
			}

			// Background
			draw::DrawRectFilled(ImVec2(x - barWidth / 2, y - barHeight), ImVec2(x + barWidth / 2, y),
				ImColor(0, 0, 0, 180), 4.0f);

			// Health bar background
			draw::DrawRectFilled(ImVec2(x - barWidth / 2 + padding, y - barHeight + shieldHeight + padding),
				ImVec2(x + barWidth / 2 - padding, y - padding),
				ImColor(20, 20, 50, 120), 3.0f);

			// Health fill
			float healthWidth = (health / 100.0f) * (barWidth - 2 * padding);
			draw::DrawRectFilled(ImVec2(x - barWidth / 2 + padding, y - barHeight + shieldHeight + padding),
				ImVec2(x - barWidth / 2 + padding + healthWidth, y - padding),
				ImColor(0, 255, 0, 255), 3.0f);

			// Shield segments
			int segments = 5;
			float segmentWidth = (barWidth - 2 * padding) / segments;
			int shieldRemaining = shield;

			ImColor shieldCracked(50, 50, 50, 120); // background for segment

			for (int i = 0; i < segments; ++i)
			{
				ImVec2 segMin(x - barWidth / 2 + padding + i * segmentWidth, y - barHeight + padding);
				ImVec2 segMax(segMin.x + segmentWidth, segMin.y + shieldHeight);

				// Draw segment background (same size as segment)
				draw::DrawRectFilled(segMin, segMax, shieldCracked, 2.0f);

				if (shieldRemaining > 0)
				{
					float widthRatio = min(shieldRemaining, maxShield / segments) / float(maxShield / segments);
					ImVec2 shieldMax(segMin.x + widthRatio * segmentWidth, segMax.y);
					draw::DrawRectFilled(segMin, shieldMax, shieldBarColor, 2.0f);
				}

				shieldRemaining -= maxShield / segments; // distribute correctly
			}
		...
		if (settings::name)
		{
			...
			// Modern/professional nametag
			draw::DrawModernNametag(head1.x, y_offset, username.c_str(),
				ImColor(20, 20, 20, 200),  // Background color (semi-transparent)
				ImColor(0, 0, 0, 255),     // Border color
				nameColor);                 // Text color
			y_offset -= 20; // slightly more spacing for rounded box
		}
		...
		if (settings::weapon_icon)
		{
			...
			BoxDimensions weaponBox = { 20, 20, 40 }; // width, height, yheight
			BoxDimensions ammoBox = { 30, 15, 35 };

			int boxX, boxY, flippedBoxY;
			int AboxX, AboxY, AflippedBoxY;

			boxX = origin.x - (weaponBox.width / 2);
			boxY = y_offset1 + weaponBox.yheight;
			flippedBoxY = boxY - weaponBox.height;

			AboxX = origin.x - (ammoBox.width / 2);
			AboxY = y_offset1 + ammoBox.yheight;
			AflippedBoxY = AboxY - ammoBox.height;

			// --- Weapon texture map (runtime textures) ---
			static std::unordered_map<int, ComPtr<ID3D11ShaderResourceView>> weaponTextureMap = {
			...
			if (it != weaponTextureMap.end()) {
				ImTextureID texture = (ImTextureID)it->second.Get(); // convert ComPtr -> raw ptr
				{
...
void render() {
	...
	if (imp(GetAsyncKeyState)(VK_INSERT) & 0x8000) settings::menu = !settings::menu;
	if (settings::menu == true)
	{
		drawmenu();

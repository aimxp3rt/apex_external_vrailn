...
__forceinline s_matrix get_view_matrix() {
	auto view_render = mem::Read<uintptr_t>(baseaddress + off::viewRender);
	if (!view_render)
		return s_matrix{};

	auto pview_matrix = mem::Read<uintptr_t>(view_render + off::viewMatrix);
	if (pview_matrix != 0) {
		return mem::Read<s_matrix>(pview_matrix);
	}
	return s_matrix{};
}
...
public:
	...
	inline int weapon_id() {
		...
		return mem::Read<int>(this->address + off::weaponHandle);
	}

	inline int ammo_in_clip() {
		if (!this->address) return -1;
		return mem::Read<int>(this->address + off::weaponAmmo);
	}

	inline float bullet_speed() {
		if (!this->address) return 0.0f;
		return mem::Read<float>(this->address + off::projectileSpeed);
	}

	inline float bullet_gravity() {
		if (!this->address) return 0.0f;
		return mem::Read<float>(this->address + off::projectileScale);
	}
...
public:
	...
	c_player(DWORD_PTR pointer) {
		...
		this->bleed_out_state = mem::Read<int>(pointer + off::bleedoutState);
	}
	...
	inline bool alive() {
		return mem::Read<int>(this->address + off::bleedoutState) == 0 && mem::Read<int>(this->address + off::lifeState) == 0;
	}
	int GetWeaponIndex()
	{
		uint64_t primary_weapons_latest = mem::Read<uint64_t>(this->address + off::weaponHandle) & 0xFFFF;
		uint64_t primary_weapons_list = mem::Read<uint64_t>(baseaddress + off::entityList + (primary_weapons_latest * 0x20));
		int weapon_index = mem::Read<int>(primary_weapons_list + off::weaponIndex); // weapon name0x1574 0x0769b4b8
		return weapon_index;
	}

	inline c_weapon weapon() {
		auto weapon_pointer = mem::Read<uintptr_t>(this->address + off::weaponHandle);
		weapon_pointer &= 0xffff;
		auto weapon_handle = mem::Read<uintptr_t>(baseaddress + off::entityList + (weapon_pointer << 5));
	...
	inline std::string s_name() {
		...
		auto index = mem::Read<uintptr_t>(this->address + off::nameIndex);

		uintptr_t NameOffset = mem::Read<uintptr_t>(baseaddress + off::nameList + ((index - 1) * 24)); //NameList=0xd350450
		if (!NameOffset)
			return "AI";
	...
	std::string GetOperator()
	{
		...
		uintptr_t pMode_Name = mem::Read< uintptr_t >(this->get_address() + 0x0030);
		if (!pMode_Name) {
			return "";
		}
		auto ModelName = mem::ReadString(pMode_Name , 64);
	...
	inline int i_health() {
		this->health = mem::Read<int>(this->address + off::health);
		return this->health;
	}

	inline bool on_ground() {
		this->flags = mem::Read<uint32_t>(this->address + off::flags);
	...
	inline int i_team() {
		this->team = mem::Read<int>(this->address + off::teamNumber);
		return this->team;
	}

	inline int i_shield() {
		this->shield = mem::Read<int>(this->address + off::shield);
		return this->shield;
	}

	inline int i_max_shield() {
		this->max_shield = mem::Read<int>(this->address + off::maxHealth);
		return this->max_shield;
	}

	inline int i_shield_type() {
		this->shield_type = mem::Read<int>(this->address + off::armorType);
	...
	inline vector3 mins() {// OFF_Collision_m_vecMins
		return mem::Read<vector3>(this->address + off::collisionVecMins);
	}

	inline vector3 maxs() {
		return mem::Read<vector3>(this->address + off::collisionVecMaxs);
	}

	inline int distance() {
		auto local_player = mem::Read<uintptr_t>(baseaddress + off::localPlayer);
		auto local_dist = mem::Read<vector3>(local_player + off::localOrigin);

		return (int)local_dist.distance(this->vec_origin());
	}

	inline vector3 vec_origin() const {
		return mem::Read<vector3>(this->address + off::localOrigin);
	}
	inline vector3 r_view_angles() {
		return mem::Read<vector3>(this->address + off::viewAngles);
	}

	inline vector3 camera_position() {
		return mem::Read<vector3>(this->address + off::cameraOrigin);
	}

	void SetViewAngles(Vector2 Angles)
	{
		mem::write<Vector2>((uintptr_t)this + off::viewAngles, Angles);
	}

	inline vector3 bone_position(uint32_t id) {
		uintptr_t bone_array = mem::Read<uintptr_t>(this->address + off::bones);
		this->origin = this->vec_origin();

		vector3 bone = vector3();
		bone.x = mem::Read<float>(bone_array + 0xCC + (id * 0x30)) + this->origin.x;
		bone.y = mem::Read<float>(bone_array + 0xDC + (id * 0x30)) + this->origin.y;
		bone.z = mem::Read<float>(bone_array + 0xEC + (id * 0x30)) + this->origin.z;
		return bone;
	}
	...
	inline bool visible(int index) {
		if (get_ms() >= (last_visible.last_check_time[index] + 10)) {
			float s_visible_time = mem::Read<float>(this->address + off::lastVisibleTime);
	...
	inline vector3 punch_angles() {
		return mem::Read<vector3>(this->address + off::punchAngles);
	}

	inline void view_angles(vector3 angles) {
		mem::write<vector3>(this->address + off::viewAngles, angles);
	}

	inline vector3 velocity() const {
		return mem::Read<vector3>(this->address + off::absVelocity);
	}
...
__forceinline std::string class_signifier(DWORD_PTR entity) {
	auto class_pointer = mem::Read<uintptr_t>(entity + off::signifierName);
	
	return mem::ReadString(class_pointer , 64);
}

inline std::string get_weapon_name(uint64_t playerEntity)
{
	uint64_t primary_weapons_latest = mem::Read<uint64_t>(playerEntity + off::weaponHandle) & 0xFFFF;
	uint64_t primary_weapons_list = mem::Read<uint64_t>(baseaddress + off::entityList + (primary_weapons_latest * 0x20));

	int weapon_index = mem::Read<int>(primary_weapons_list + off::weaponIndex);

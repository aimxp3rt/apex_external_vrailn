...
#include "../../../entry/engine/cheat/config.h"
#include "../../protection/oxorany.h"
...
void HotkeyButton(int aimkey, void* changekey, int status)
{
	...
	if (preview_value == NULL)
		aimkeys = (xxx("Select Key"));
	else
		aimkeys = preview_value;

	if (status == 1)
	{
		aimkeys = (xxx("Press the key"));
	}
...
void Hotkeytrigger(int triggerkey, void* changekey, int status)
{
	...
	if (preview_value == NULL)
		triggerkeys = (xxx("Select Key"));
	else
		triggerkeys = preview_value;

	if (status == 1)
	{
		triggerkeys = (xxx("Press the key"));
	}

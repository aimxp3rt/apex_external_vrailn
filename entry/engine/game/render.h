...
namespace draw {
	...
	inline void DrawRectFilled(const ImVec2& min, const ImVec2& max, ImColor color, float rounding = 0.0f, ImDrawFlags flags = 0)
	{
	...
	inline void DrawSeerHealth(float x, float y, int shield, int max_shield, int armorType, int health,
		float width = 80.0f, float height = 12.0f)
	{
		const int segments = 5;
		const float padding = 1.5f;

		float shieldHeight = height * 0.35f;
		float healthHeight = height - shieldHeight;

		ImColor bgColor = ImColor(0, 0, 0, 180);
		ImColor healthColor = ImColor(0, 255, 0, 255);
		ImColor shieldColor = GetShieldColor(armorType);
		ImColor shieldEmptyColor = ImColor(80, 80, 80, 200);

		// Full background
		ImVec2 bgMin(x - width / 2, y - height);
		ImVec2 bgMax(x + width / 2, y);
		DrawRectFilled(bgMin, bgMax, bgColor, 4.0f); // Rounded corners

		// Health bar background
		ImVec2 healthMin(bgMin.x + padding, bgMin.y + shieldHeight + padding);
		ImVec2 healthMax(bgMax.x - padding, bgMax.y - padding);
		DrawRectFilled(healthMin, healthMax, ImColor(25, 25, 50, 150), 3.0f);

		// Health fill
		float healthWidth = (float)health / 100.0f * (width - 2 * padding);
		DrawRectFilled(healthMin, ImVec2(healthMin.x + healthWidth, healthMax.y), healthColor, 3.0f);
		...
		for (int i = 0; i < segments; ++i) {
			ImVec2 segMin(healthMin.x + i * segmentWidth, healthMin.y - shieldHeight);
			ImVec2 segMax(segMin.x + segmentWidth, segMin.y + shieldHeight);

			// Draw empty shield
			DrawRectFilled(segMin, segMax, shieldEmptyColor, 2.0f);

			// Fill with actual shield
			if (remainingShield > 0) {
				float filledWidth = (float)min(remainingShield, max_shield / segments) / (max_shield / segments) * segmentWidth;
				DrawRectFilled(segMin, ImVec2(segMin.x + filledWidth, segMax.y), shieldColor, 2.0f);
			}

			remainingShield -= max_shield / segments;
		}
	}
	...
	inline void DrawModernNametag(float x, float y, const char* name,
		ImColor bgColor = ImColor(20, 20, 20, 200),
		ImColor borderColor = ImColor(0, 0, 0, 255),
		ImColor textColor = ImColor(255, 255, 255, 255))
	{
		...
		drawList->AddRectFilledMultiColor(
			boxMin, boxMax,
			ImColor(bgColor.Value.x * 0.8f, bgColor.Value.y * 0.8f, bgColor.Value.z * 0.8f, bgColor.Value.w),
			bgColor, bgColor, ImColor(bgColor.Value.x * 0.8f, bgColor.Value.y * 0.8f, bgColor.Value.z * 0.8f, bgColor.Value.w)
		);

		// Draw border
		drawList->AddRect(boxMin, boxMax, borderColor, rounding, 0, 1.5f);

		// Draw text shadow for depth
		ImVec2 textPos(x - textSize.x / 2 + 1, y - textSize.y / 2 + 1);
		drawList->AddText(font, fontSize, textPos, ImColor(0, 0, 0, 150), name);
